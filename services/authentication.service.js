...
const userRegister = async (user) => {
    try {
        ...
        error?.code === 1100 && error?.keyPattern?.username
            ? (errorMessage = "Username already exist")
            : null;
        error?.code === 1100 && error?.keyPattern?.email
...
const userLogin = async (user) => {
    try {
        ...
        if (userObject) {
            let isPasswordVerified = bcrypt.compare(user?.password, userObject?.password)
